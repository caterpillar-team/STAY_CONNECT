...
public class RoomInfoInitializer implements CommandLineRunner {
  ...
  for (int i = 1; i <= TOTAL_ACCOMMODATIONS; i++) {
      Accommodation accommodation = accommodationRepository.findById((long) i)
          .orElseThrow(() -> new NoSuchElementException("Accommodation not found for id: "));
  ...
  // private Accommodation getRandomAccommodation() {
  //   int randomIndex = (int) (Math.random() * 30) + 1; // 1에서 30까지의 숙소 인덱스
  //   return accommodationRepository.findById((long) randomIndex).orElseThrow(() -> new NoSuchElementException("Accommodation not found for id: " + randomIndex));
  // }
}